# coding: utf-8

"""
    Unifi API

    Unifi Controller API

    The version of the OpenAPI document: 8.0.26
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from openapi_client.models.setting_radius_response import SettingRadiusResponse

class TestSettingRadiusResponse(unittest.TestCase):
    """SettingRadiusResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SettingRadiusResponse:
        """Test SettingRadiusResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SettingRadiusResponse`
        """
        model = SettingRadiusResponse()
        if include_optional:
            return SettingRadiusResponse(
                data = [
                    openapi_client.models.setting_radius.SettingRadius(
                        _id = '', 
                        accounting_enabled = True, 
                        acct_port = 56, 
                        attr_hidden = True, 
                        attr_hidden_id = '', 
                        attr_no_delete = True, 
                        attr_no_edit = True, 
                        auth_port = 56, 
                        configure_whole_network = True, 
                        enabled = True, 
                        interim_update_interval = 56, 
                        key = '', 
                        site_id = '', 
                        tunneled_reply = True, 
                        x_secret = '', )
                    ],
                meta = openapi_client.models.meta.Meta(
                    msg = '', 
                    rc = '', )
            )
        else:
            return SettingRadiusResponse(
        )
        """

    def testSettingRadiusResponse(self):
        """Test SettingRadiusResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
